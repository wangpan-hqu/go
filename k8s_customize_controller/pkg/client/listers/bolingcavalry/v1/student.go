...
type VMLister interface {
	// List lists all Students in the indexer.
	// Objects returned here must be treated as read-only.
	List(selector labels.Selector) (ret []*kv1.VirtualMachine, err error)
	// Students returns an object that can list and get Students.
...
func (s *vmLister) List(selector labels.Selector) (ret []*kv1.VirtualMachine, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*kv1.VirtualMachine))
	})
	return ret, err
}
...
type VMNamespaceLister interface {
	...
	// Get retrieves the Student from the indexer for a given namespace and name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*kv1.VirtualMachine, error)
	StudentNamespaceListerExpansion
}
...
func (s vmNamespaceLister) List(selector labels.Selector) (ret []*kv1.VirtualMachine, err error) {
	err = cache.ListAllByNamespace(s.indexer, s.namespace, selector, func(m interface{}) {
		ret = append(ret, m.(*kv1.VirtualMachine))
	})
	return ret, err
...
func (s vmNamespaceLister) Get(name string) (*kv1.VirtualMachine, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	...
	return obj.(*kv1.VirtualMachine), nil
}
